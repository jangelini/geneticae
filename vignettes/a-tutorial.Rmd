...
as have the vectors with greater length. The cultivars _m12_ and _Kat_ present
similar interaction patterns because of their closeness and very different from
_Ann_ and _Aug_. The closeness between _Dia_  and BH93 indicates strong positive
association between the cultivar and the environment, that is to say, it is a
...
performance in all locations except OA93 and KE93, since the acute angles.
Environmental vectors length gives a measure of the environment's ability to
...
axis.
The performance of different genotypes in OA93 can be done setting `type =
"Selected Environment"` and providing the name of the environment (OA93) in
...
_Cas_ it was more yielding than _Kat_ in all environments as all the
environments are on the same side of the perpendicular line as _Cas_.
...
Selecting cultivars within each mega-environments will be an issue among plant
breeders. Figure 6, clearly suggests that _Zav_ was the best cultivar for OA93
and KE93, and _Fun_ was the best cultivar for the other locations. However,
...
*genotype-focused scaling*, obtained indicatig `SVP = "row"` in `GGEmodel()`,
provides a superior means for visualizing both mean performance and stability of
...
Figure 8 allow comparing the cultivars with the "ideal" one, with the highest
