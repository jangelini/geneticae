...
test_that("Several tests for GGE_Model function", {
  ...
  expect_that(GGEmodel(yan.winterwheat,genotype="gen",environment="env", response="yield"), is_a("GGEModel") )

  expect_error(GGEmodel(genotype="gen",environment="env", response="yield"),
               "Need to provide Data data frame")

  expect_equal(GGEmodel(yan.winterwheat,genotype="gen", environment="env", response="yield"),
              GGEmodel(yan.winterwheat))

  expect_identical(yan.winterwheat, na.omit(yan.winterwheat))

  expect_error(GGEmodel(dat2,genotype="gen",environment="env", response="yield"),
