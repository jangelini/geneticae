...
#'  using the output of the \code{\link{rSREGModel}} function.
...
#'
#'@param rSREGModel An object of class \code{rSREGModel}.
...
rSREGPlot<-function(rSREGModel,type="Biplot",d1=1,d2=2, selectedE=NA , selectedG=NA,selectedG1=NA,selectedG2=NA,
                  colGen="gray47",colEnv="darkred",colSegment="gray30",colHull="gray30",sizeGen=6,sizeEnv=6,largeSize=4.5,axis_expand=1.2,
  ...
  coordgenotype=rSREGModel$coordgenotype[,c(d1,d2)];coordenviroment=rSREGModel$coordenviroment[,c(d1,d2)]
  varexpl=rSREGModel$varexpl[c(d1,d2)];labelgen=rSREGModel$labelgen;
  labelenv=rSREGModel$labelenv;labelaxes=rSREGModel$labelaxes[c(d1,d2)];Data=rSREGModel$Data
  SVP=rSREGModel$SVP
  ...
  labelaxes_x <- paste("PC", d1) 
  labelaxes_y <- paste("PC", d2)
  
  if(axislabels == TRUE){
    GGE1 <- GGE1 + 
      xlab(paste(labelaxes_x, " (", format(varexpl[1], nsmall = 2), "%)", sep = "")) +
      ylab(paste(labelaxes_y, " (", format(varexpl[2], nsmall = 2), "%)", sep = "")) +
      theme(
  ...
  if(type=="Selected Environment"){
    ...
    GGE2<-GGE1+
      ...
      geom_segment(xend=0,yend=0,col=alpha(colEnv,0.5),data=subset(plotdata,type=="environment"&label==selectedE),
                   arrow =arrow(ends ="first",length=unit(0.2,"inches") ),linewidth=1)+
  ...
  if(type=="Selected Genotype"){
    ...
    GGE2<-GGE1+
      geom_abline(slope=coordgenotype[vgenotype, 2]/coordgenotype[vgenotype,1],intercept=0,col=alpha(colSegment,0.5),linewidth=0.7)+
      geom_abline(slope=-coordgenotype[vgenotype, 1]/coordgenotype[vgenotype,2],intercept=0,col=alpha(colSegment,0.5),linewidth=0.7)+
      geom_segment(xend=0,yend=0,col=alpha(colSegment,0.5),data=subset(plotdata,type=="genotype"&label==selectedG),
                   arrow =arrow(ends ="first",length=unit(0.15,"inches") ),linewidth=1)+
  ...
  if(type=="Relationship Among Environments"){
    ...
    GGE2<-GGE1+
      geom_segment(xend=0,yend=0,col=alpha(colEnv,0.5),data=subset(plotdata,type=="environment"),
                   arrow =arrow(ends ="first",length=unit(0.1,"inches") ),linewidth=0.7, linetype =2)+
  ...
  if(type=="Comparison of Genotype"){
    ...
    GGE2<-GGE1+geom_point(data=subset(plotdata,type=="genotype"&label%in%c(selectedG1,selectedG2)))+
      annotate("segment", 
               x = plotdata$d1[plotdata$label==selectedG1 & plotdata$type=="genotype"],
               ...
               yend = plotdata$d2[plotdata$label==selectedG2 & plotdata$type=="genotype"], 
               col=colGen, linewidth=1) + 
  ...
  if(type=="Which Won Where/What"){
    ...
    GGE2<-GGE1+geom_polygon(aes(x=X1,y=X2),data=www,fill=NA,col=colHull,linewidth=0.7)+
      geom_segment(data=segs,aes(x=X1,y=X2),xend=0,yend=0,linetype=2,col=colSegment,linewidth=0.7) +
  ...
  if(type=="Discrimination vs. representativeness"){
    ...
    GGE2<-GGE1+geom_arc(aes(r=radio,x0=x0,y0=y0,start=start,end=end),data=circles,col=colSegment,inherit.aes=F, na.rm=TRUE,linetype =2)+
      geom_segment(xend=0,yend=0,x=mean(coordenviroment[,1]),y=mean(coordenviroment[,2]),arrow =arrow(ends ="first",length=unit(0.1,"inches") ),linewidth=1,col=colSegment)+
  ...
  if(type=="Ranking Environments"){
    ...
    GGE2<-GGE1+
      geom_abline(intercept=0, slope=med2/med1, col=colSegment, linewidth=0.7)+
      geom_abline(intercept=0, slope=-med1/med2, col=colSegment, linewidth=0.7)+
      geom_arc(aes(r=radio, x0=x0, y0=y0, start=start, end=end), 
               data=circles, col=colSegment, inherit.aes=F, na.rm=TRUE, linetype=2)+
      annotate("segment", x=0, y=0, xend=xcoord, yend=ycoord, 
               arrow = arrow(length=unit(0.15,"inches")), 
               linewidth=1, col=colSegment)+
    geom_text(aes(label=label), show.legend = FALSE, 
              data=subset(plotdata, type=="environment"), col=colEnv, size=sizeEnv)
  ...
  if(type=="Mean vs. Stability"){
    ...
    GGE2<-GGE1+
      ...
      annotate("segment", x=0, y=0, xend=med1, yend=med2, 
               arrow = arrow(length=unit(0.15,"inches")), 
               linewidth=1, col=colSegment) +
    geom_text(aes(label=label),show.legend = FALSE,data=subset(plotdata,type=="genotype"),col=colGen,size=sizeGen)+
      geom_text(aes(label=label),show.legend = FALSE,data=subset(plotdata,type=="environment"),col=colEnv,size=sizeEnv)
  ...
  if(type=="Ranking Genotypes"){
    ...
    GGE2<-GGE1+
      geom_abline(intercept=0,slope=med2/med1,col=colGen, linewidth=0.7)+
      geom_abline(intercept=0,slope=-med1/med2,col=colGen, linewidth=0.7)+
      geom_arc(aes(r=radio,x0=x0,y0=y0,start=0,end=pi*2),data=circles,col=colSegment,inherit.aes=F,linetype =2, na.rm=TRUE)+
      annotate("segment", x=0, y=0, xend=coordx, yend=coordy, 
               arrow = arrow(length=unit(0.15,"inches")), 
               linewidth=1, col=colGen) +
      geom_text(aes(label=label),show.legend = FALSE,data=subset(plotdata,type=="genotype"),col=colGen,size=sizeGen)+
      geom_text(aes(label=label),show.legend = FALSE,data=subset(plotdata,type=="environment"),col=colEnv,size=sizeEnv)
